...
template <typename BitsT = detail::Bits<uint64_t, uint64_t>,
          size_t SCALE = 1>
class SparseBitvectorUnorderedImpl {
    // we use unordered vector to store the bits
...
public:
    SparseBitvectorUnorderedImpl() = default;
    SparseBitvectorUnorderedImpl(size_t i) { set(i); } // singleton ctor

    SparseBitvectorUnorderedImpl(const SparseBitvectorUnorderedImpl&) = default;
    SparseBitvectorUnorderedImpl(SparseBitvectorUnorderedImpl&&) = default;
    ...
    // this is the union operation
    bool merge(const SparseBitvectorUnorderedImpl& rhs) {
        bool changed = false;
        // FIXME: this is inefficient
        for (size_t i : rhs) {
            // if we changed a bit from false to true,
